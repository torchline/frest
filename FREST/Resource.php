...
require_once(dirname(__FILE__) . '/Enum/Filter.php');
require_once(dirname(__FILE__) . '/Setting/Settings.php');
require_once(dirname(__FILE__) . '/Func/Functions.php');
require_once(dirname(__FILE__) . '/Result/ResourceFunc.php');
...
abstract class Resource {
	...
	/**
	 * Whether or not to allow Request searching for a set of Resource (e.g. /api/users)
	 *
	 * @var int (optional)
	 */
	 ...
	 * 
	 * Note: Normally only one table is used since Resource are usually defined in a
	...
	/**
	 * An associative array with keys indicating the pseudo name or "alias" of a property
	 * of the resource that can be used to order Resource in a GET request (corresponding 
	...
	/**
	 * @param Setting\Field $fieldSetting
	 * 
	 * @return string
	 */
	public function getIDField(&$fieldSetting = NULL) {
		/** @var Setting\Table $firstTableSetting */
	...
	public function modifyReadSettings($settingsToModify) {
		...
		/** @var Setting\Read $settingToModify */
	...
	public function modifyConditionSettings($settingsToModify) {
		...
		/** @var Setting\Condition $settingToModify */
		foreach ($settingsToModify as $settingToModify) {
			$key = $settingToModify->getAlias();
	...
	public function modifyOrderSettings($settingsToModify) {
		...
		/** @var Setting\Order $settingToModify */
		foreach ($settingsToModify as $settingToModify) {
			$key = $settingToModify->getAlias();
	...
	public function modifyCreateSettings($settingsToModify) {
		...
		/** @var Setting\Create $settingToModify */
		foreach ($settingsToModify as $settingToModify) {
			$key = $settingToModify->getAlias();
	...
	public function modifyUpdateSettings($settingsToModify) {
		...
		/** @var Setting\Update $settingToModify */
		foreach ($settingsToModify as $settingToModify) {
			$key = $settingToModify->getAlias();
	...
	public function getReadSettings() {
		if (!isset($this->readSettings)) {
			...
			foreach ($this->tableSettings as $tableSetting) {
				...
				/** @var Setting\Field $fieldSetting */
				foreach ($fieldSettings as $fieldSetting) {
					$readSettings[] = Setting\Settings::readField($fieldSetting->getAlias());
				}
			}
	...
	public function getOrderSettings() {
		if (!isset($this->orderSettings)) {
			...
			foreach ($this->tableSettings as $tableSetting) {
				...
				/** @var Setting\Field $fieldSetting */
				foreach ($fieldSettings as $fieldSetting) {
					$orderSettings[] = Setting\Settings::order($fieldSetting->getAlias());
				}
			}
		...
		if (!isset($this->createSettings)) {
			...
			foreach ($this->tableSettings as $tableSetting) {
				...
				foreach ($fieldSettings as $fieldSetting) {
					...
					if ($field !== $idField) {
						$createSettings[] = Setting\Settings::create($alias);
					}
				}
			}
		...
		if (!isset($this->updateSettings)) {
			...
			foreach ($this->tableSettings as $tableSetting) {
				...
				foreach ($fieldSettings as $fieldSetting) {
					...
					if ($field !== $idField) {
						$updateSettings[] = Setting\Settings::update($alias);
					}
				}
			}
